...
def get_feature_columns() -> list[str]:
    """
    Define the predictor columns for the linear regression model.

    These are feature-selection tuning knobs.
    You can later add or remove features here to compare performance.
    """
    return [
        "hour_of_day",
        "day_of_week",
...
def plot_actual_vs_predicted(
    ...
    Increase it for a longer view, or decrease it for a cleaner figure.
    """
    ...
    # Plot actual and predicted demand on the same axes
    plt.figure(figsize=(12, 5))
    plt.plot(plot_df["hour"], plot_df["actual"], label="Actual")
    plt.plot(plot_df["hour"], plot_df["predicted"], label="Predicted")
    plt.title(f"Linear Regression: Actual vs Predicted ({dataset_name})")
    plt.xlabel("Time")
...
def plot_coefficients(model: LinearRegression, feature_cols: list[str], output_file: Path) -> None:
    """
    Plot the learned coefficients so we can see how each feature influences prediction.
    """
    # Make sure the figure output folder exists
    FIGURE_DIR.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    # Predict session count for train, validation, and test sets
    ...
    plot_actual_vs_predicted(
        ...
        val_pred,
        "Validation",
        FIGURE_DIR / "linear_regression_val_actual_vs_pred.png",
        n_points=300,
    )
    ...
    plot_actual_vs_predicted(
        ...
        test_pred,
        "Test",
        FIGURE_DIR / "linear_regression_test_actual_vs_pred.png",
        n_points=300,
    )
